...
if (viewLink) viewLink.href = `index.html?community=${encodeURIComponent(adminCommunityId)}`;
...
loginForm.addEventListener('submit', async (e) => {
  ...
  if (window.EventsBoard.hasSupabaseConfig()) {
    ...
    if (error) {
      loginError.textContent = 'הכניסה נכשלה. בדוק אימייל וסיסמה.';
  ...
  if (password === (CONFIG.adminPassword || 'sparkco-admin')) {
    ...
    loginError.textContent = 'הסיסמה אינה נכונה.';
    loginError.classList.remove('hidden');
  }
});
...
function fillForm(event) {
  ...
  document.getElementById('category').value = event.category || 'מפגש קהילה';
  document.getElementById('status').value = event.status || 'פתוח';
...
function resetForm() {
  ...
  document.getElementById('category').value = 'מפגש קהילה';
  document.getElementById('status').value = 'פתוח';
...
async function renderAdminList() {
  ...
  adminStatus.textContent = events.length ? `${events.length} אירועים במערכת` : 'אין אירועים עדיין';
    ...
    return `
      <article class="admin-event-item" data-id="${event.id}">
        <header>
          ...
          <span class="badge">${event.isPublished ? 'מפורסם' : 'מוסתר'}</span>
        </header>
        <p>${escapeHtml(d.full)} · ${escapeHtml(window.EventsBoard.timeRange(event))} · ${escapeHtml(event.category || '')}</p>
        <div class="admin-item-actions">
          <button class="small-btn edit-event" data-id="${event.id}">עריכה</button>
          <button class="small-btn duplicate-event" data-id="${event.id}">שכפול</button>
        </div>
      </article>
    `;
  }).join('');
  ...
  document.querySelectorAll('.duplicate-event').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const event = all.find(e => e.id === btn.dataset.id);
      if (!event) return;
      const copy = { ...event, id: undefined, title: `${event.title} - עותק`, isFeatured: false };
...
eventForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    resetForm();
    await renderAdminList();
    adminStatus.textContent = 'האירוע נשמר בהצלחה';
  } catch (err) {
    adminStatus.textContent = `שגיאה בשמירה: ${err.message || err}`;
  }
});
...
deleteBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    resetForm();
    await renderAdminList();
    adminStatus.textContent = 'האירוע נמחק';
  } catch (err) {
    adminStatus.textContent = `שגיאה במחיקה: ${err.message || err}`;
  }
});
